...
SOURCE = ROOT / "source" / "Сезон 6_Все публикации_Английский через истории_ФИНАЛ.docx"
OUTPUT = ROOT / "content" / "season6.json"
START_DATE = date(2027, 2, 1)
TIMEZONE = "Europe/Moscow"
CHANNEL = "@english_story_a1a2"
COURSE_NAME = "Английский через истории"
WEEK_IMAGES = {day: f"images/season6/week{week:02d}.png" for week, day in enumerate((7, 14, 21, 28), 1)}
NO_PRACTICE_AUDIO = {2, 4, 6, 9, 10, 12, 13, 16, 17, 20, 23, 25, 26, 27}
SUBSECTION_PREFIXES = ("🎬", "🗣", "⭐", "🎧")
DIALOGUE_PREFIXES = ("👩 ", "👨 ", "🧑 ")
DIALOGUE_INDENT = "&#160;" * 5
...
def build() -> None:
    ...
    assert sorted(day_starts) == list(range(1, 29))
    final_polls_start = texts.index("28 ФЕВРАЛЯ • 14:00")
    congrats_start = texts.index("28 ФЕВРАЛЯ • 16:00")
    events = []
    ...
    for day in range(1, 29):
        ...
        morning_start = next(i for i, value in enumerate(section_texts) if value.startswith("🌱 Сезон 6"))
        ...
        morning_steps = [
            ...
            {"id": "audio", "type": "audio", "path": f"audio/season6/morning/day{day:02d}.mp3", "title": f"Сезон 6 · День {day} · Утро", "performer": COURSE_NAME},
        ]
        if day in WEEK_IMAGES:
            morning_steps.insert(0, {"id": "image", "type": "photo", "path": WEEK_IMAGES[day]})
        events.append({"id": f"s6_day{day:02d}_morning", "date": event_date, "time": "07:00", "retry_until": "09:00", "steps": morning_steps})
        ...
        practice_path = ROOT / f"audio/season6/practice/day{day:02d}.mp3"
        if practice_path.exists():
            steps.append({"id": "audio", "type": "audio", "path": f"audio/season6/practice/day{day:02d}.mp3", "title": f"Сезон 6 · День {day} · Практика", "performer": COURSE_NAME})
        ...
        events.append({"id": f"s6_day{day:02d}_practice", "date": event_date, "time": "12:00", "retry_until": "14:00", "steps": steps})
        ...
        if bonus_start is not None:
            if day == 21:
                label = next(i for i in range(bonus_start + 1, len(section)) if section_texts[i].startswith("Telegram Quiz Poll"))
                options_index = next(i for i in range(label + 1, len(section)) if section_texts[i].startswith(("✅", "❌")))
                bonus_steps = [
                    {"id": "text", "type": "message", "text": join_paragraphs(section[bonus_start + 1:label])},
                    make_poll("poll", "\n".join(value for value in section_texts[label + 1:options_index] if value), option_block(section_texts, options_index)),
                ]
            else:
                bonus_steps = [{"id": "text", "type": "message", "text": join_paragraphs(bonus_body(section, bonus_start))}]
                if day == 7:
                    bonus_steps.append({"id": "audio", "type": "audio", "path": "audio/season6/bonus/day07_adam.mp3", "title": "Сезон 6 · День 7 · Голосовое от Адама", "performer": COURSE_NAME})
            events.append({"id": f"s6_day{day:02d}_bonus", "date": event_date, "time": "18:00", "retry_until": "20:00", "steps": bonus_steps})
    ...
    for number, head in enumerate(poll_heads, 1):
        ...
        if number == 7:
            final_steps.append({"id": "poll07_audio", "type": "audio", "path": "audio/season6/final/final_poll_07.mp3", "title": "Сезон 6 · Итоговый опрос · Задание 7", "performer": COURSE_NAME})
    ...
    events.append({"id": "season6_final_polls", "date": "2027-02-28", "time": "14:00", "retry_until": "16:00", "steps": final_steps})

    congrats_section = paragraphs[congrats_start:]
    congrats_texts = [p.text.strip() for p in congrats_section]
    body_start = next(i for i, value in enumerate(congrats_texts) if value.startswith("🎉 SEASON 6 COMPLETE"))
    photo_marker = next(i for i, value in enumerate(congrats_texts) if value.startswith("📸 ФИНАЛЬНОЕ ФОТО"))
    events.append({"id": "season6_congratulations", "date": "2027-02-28", "time": "16:00", "retry_until": "18:00", "steps": [
        {"id": "image", "type": "photo", "path": "images/season6/final.png"},
        {"id": "text", "type": "message", "text": join_congratulations(congrats_section[body_start:photo_marker])},
    ]})

    final_bonus_start = congrats_start + next(i for i, value in enumerate(congrats_texts) if value.startswith("ВЕЧЕРНИЙ БОНУС"))
    final_bonus_end = next((i for i in range(final_bonus_start + 1, len(paragraphs)) if paragraphs[i].text.strip().startswith(("Служебно:", "ПРОИЗВОДСТВЕННЫЕ ЗАМЕТКИ"))), len(paragraphs))
    events.append({"id": "s6_day28_bonus", "date": "2027-02-28", "time": "18:00", "retry_until": "20:00", "steps": [
        {"id": "text", "type": "message", "text": join_paragraphs(paragraphs[final_bonus_start + 1:final_bonus_end])}
    ]})

    practice_days = {int(e["id"][6:8]) for e in events if e["id"].startswith("s6_day") and e["id"].endswith("_practice") for step in e["steps"] if step["type"] == "audio"}
    assert practice_days == set(range(1, 29)) - NO_PRACTICE_AUDIO
    ...
    payload = {"meta": {"course": COURSE_NAME, "season": 6, "start_date": "2027-02-01", "end_date": "2027-02-28", "timezone": TIMEZONE, "channel": CHANNEL, "source_sha256": sha256(SOURCE), "audio_files": 44, "image_files": 5}, "events": events, "assets": {relative: sha256(ROOT / relative) for relative in asset_paths}}
