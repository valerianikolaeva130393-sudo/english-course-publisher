...
SOURCE = ROOT / "source" / "Сезон 3_Все публикации_Английский через истории_ФИНАЛ.docx"
OUTPUT = ROOT / "content" / "season3.json"
...
WEEK_IMAGES = {
    7: "images/season3/week01.png",
    14: "images/season3/week02.png",
    21: "images/season3/week03.png",
    28: "images/season3/week04.png",
}
...
def build() -> None:
    ...
    final_polls_start = texts.index("30 НОЯБРЯ • 14:00")
    congrats_start = texts.index("30 НОЯБРЯ • 16:00")
    events: list[dict] = []
    ...
    for day in range(1, 31):
        ...
        morning_start = next(i for i, value in enumerate(section_texts) if value.startswith("🌱 Сезон 3"))
        ...
        morning_steps = [
            {"id": "text", "type": "message", "text": morning_text},
            {
                "id": "audio",
                "type": "audio",
                "path": f"audio/season3/morning/day{day:02d}.mp3",
                "title": f"Сезон 3 · День {day} · Утро",
        ...
        practice_audio_marker = next(
            (
                i
                for i in range(practice_start, poll_label)
                if section_texts[i].startswith("🎧 Аудио:")
            ),
            poll_label,
        )
        practice_text = join_paragraphs(section[practice_start:practice_audio_marker])
        ...
        practice_audio = ROOT / f"audio/season3/practice/day{day:02d}.mp3"
        if practice_audio.exists():
            practice_steps.append(
                {
                    "id": "audio",
                    "type": "audio",
                    "path": f"audio/season3/practice/day{day:02d}.mp3",
                    "title": f"Сезон 3 · День {day} · Практика",
                    "performer": COURSE_NAME,
                }
            )
            ...
            final_steps.append(
                {
                    "id": "poll07_audio",
                    "type": "audio",
                    "path": "audio/season3/final/final_poll_07.mp3",
                    "title": "Сезон 3 · Итоговый опрос · Задание 7",
                    "performer": COURSE_NAME,
                }
            )
            question_parts = [
    ...
    congrats_body_start = next(i for i, value in enumerate(congrats_texts) if value.startswith("🎉 SEASON 3 COMPLETE"))
    ...
    events.append(
        {
            "id": "season3_congratulations",
            "date": "2026-11-30",
            "time": "16:00",
            "retry_until": "18:00",
            "steps": [
                {"id": "image", "type": "photo", "path": "images/season3/final.png"},
    ...
    practice_days = {
        int(event["id"][6:8])
        for event in events
        if event["id"].startswith("s3_day") and event["id"].endswith("_practice")
