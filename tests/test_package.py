...
def verify_schedule(season1: dict, season2: dict) -> None:
...
def main() -> None:
    ...
    verify_schedule(season1, season2)

    combined = load_all_content()
    assert combined["meta"]["seasons"] == [1, 2]
    assert len(combined["events"]) == 126
    assert len({event["id"] for event in combined["events"]}) == 126
    assert combined["events"][0]["date"] == "2026-09-01"
    assert combined["events"][-1]["date"] == "2026-10-31"

    print(
        "Package QA passed: 2 seasons, 126 events, 314 Telegram steps, "
        "105 audio, 10 images, 75 polls"
    )
